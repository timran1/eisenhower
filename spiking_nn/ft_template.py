...
class SpikingNN_FT_Template(FeatureTree):
    ...
    def __init__(self):
                ...
                Feature("nodetypes_set", "Node Types Set", "", [
                    Feature("node", "Node", "", [
                        Feature("data_mov", "Data Movement", "", [
                            Feature("num_gran",    "Granularity",        "No. of movement packet sizes"),
        ...
        try:
            ...
            node.find_sub_f("node.data_mov.num_gran")[0].init(    stage_mask=self.MASK_SINGL_ONLY_SET)
            ...
            # Node
            [f.init(num_dims=0) for f in self.find_sub_f("system.nodetypes_set.node.data_mov.num_gran")]
